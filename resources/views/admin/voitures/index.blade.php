...
<head>
    ...
    <title>RenaultOcaz - Gestion des Voitures</title>
    <!-- Fonts -->
...
<body>
    ...
    <div class="container container-main">
        <div class="page-header">
            <h1 class="page-title">Gestion des Voitures</h1>
            <a href="{{ route('admin.voitures.create') }}" class="btn-action">
                        ...
                        <tbody>
                            @forelse($voitures as $voiture)
                                <tr>
                                    <td>
                                        @if($voiture->image)
                                            <img src="{{ asset('storage/' . $voiture->image) }}" alt="{{ $voiture->marque }}" class="img-thumbnail" style="width: 60px; height: 45px; object-fit: cover;">
                                        @else
                                            <div class="bg-light d-flex align-items-center justify-content-center" style="width: 60px; height: 45px; border-radius: 8px;">
                                    ...
                                    <td>
                                        @if($voiture->en_promotion && $voiture->prix_promotion)
                                            <span class="text-decoration-line-through text-muted me-1">{{ number_format($voiture->prix, 2, ',', ' ') }} €</span>
                                            <span class="text-danger fw-bold">{{ number_format($voiture->prix_promotion, 2, ',', ' ') }} €</span>
                                            @if($voiture->date_fin_promotion)
                                                <br><small class="text-muted">Jusqu'au {{ \Carbon\Carbon::parse($voiture->date_fin_promotion)->format('d/m/Y') }}</small>
                                            @endif
                                        @else
                                            {{ number_format($voiture->prix, 2, ',', ' ') }} €
                                        @endif
                                    </td>
                                    <td>{{ $voiture->entreprise->nom ?? 'N/A' }}</td>
                                    <td>
                                        @php
                                            $statusClasses = [
                                                ...
                                                'en_vente' => 'bg-info',
                                                'vendu' => 'bg-secondary',
                                                'indisponible' => 'bg-warning'
                                            ];
                                            $statusIcons = [
                                                'disponible' => 'bi-check-circle',
                                                'en_vente' => 'bi-tag',
                                                'vendu' => 'bi-check-circle-fill',
                                                'indisponible' => 'bi-x-circle'
                                            ];
                                        @endphp
                                        <span class="badge {{ $statusClasses[$voiture->statut] ?? 'bg-secondary' }} d-flex align-items-center" style="gap: 5px;">
                                            <i class="bi {{ $statusIcons[$voiture->statut] ?? 'bi-question-circle' }}"></i>
                                            {{ ucfirst(str_replace('_', ' ', $voiture->statut)) }}
                                        </span>
                                    </td>
                                    <td class="text-center">
                                        @if($voiture->en_promotion)
                                            <span class="badge bg-danger" data-bs-toggle="tooltip" data-bs-placement="top" title="Promotion en cours">
                                                <i class="bi bi-tag"></i>
                                            </span>
                                        @else
                                    ...
                                    <td>
                                        <button class="action-btn" type="button" onclick="showActionModal({{ $voiture->id }})">
                                ...
                                <tr>
                                    <td colspan="9" class="text-center py-4">
                                        <div class="text-muted">
                                            ...
                                            <p class="mt-2 mb-0">Aucune voiture trouvée</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
    ...
    <script>
        ...
        function showActionModal(voitureId) {
            ...
            viewBtn.onclick = function() {
                window.location.href = `/admin/voitures/${voitureId}`;
            };
            ...
            editBtn.onclick = function() {
                window.location.href = `/admin/voitures/${voitureId}/edit`;
            };
            ...
            deleteBtn.onclick = function() {
                if (confirm('Êtes-vous sûr de vouloir supprimer cette voiture ?')) {
                    ...
                    form.action = `/admin/voitures/${voitureId}`;
                    form.submit();
                }
            };
