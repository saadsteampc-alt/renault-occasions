...
<head>
    ...
    <title>RenaultOcaz - Demandes Clients</title>
    <!-- Fonts -->
...
<body>
    ...
    <div class="container container-main">
        <h1 class="page-title">Demandes Clients</h1>
        
        <ul class="nav nav-tabs mb-3" id="demandesTab" role="tablist">
            <li class="nav-item" role="presentation">
                <button class="nav-link active" id="en-attente-tab" data-bs-toggle="tab" data-bs-target="#en-attente" type="button" role="tab">
                    En attente <span class="badge bg-danger">{{ $demandesEnAttente->count() }}</span>
                </button>
            </li>
            <li class="nav-item" role="presentation">
                        ...
                        <tbody>
                            @forelse($demandesEnAttente as $demande)
                                <tr>
                                    ...
                                    <td>
                                        <strong>{{ $demande->client->nom }} {{ $demande->client->prenom }}</strong><br>
                                        <small class="text-muted">{{ $demande->client->email }}</small>
                                    </td>
                                    <td>
                                        {{ $demande->voiture->marque }} {{ $demande->voiture->modele }}<br>
                                        <small class="text-muted">{{ $demande->voiture->annee }} - {{ number_format($demande->voiture->kilometrage, 0, ',', ' ') }} km</small>
                                    </td>
                                    <td>
                                        <i class="bi bi-telephone"></i> {{ $demande->client->telephone }}<br>
                                        <small class="text-muted">{{ Str::limit($demande->client->adresse, 30) }}</small>
                                    </td>
                                    <td>
                                        <a href="{{ route('vendeur.demandes.show', $demande) }}" class="btn btn-info btn-sm" title="Voir">
                                            <i class="bi bi-eye"></i>
                                        </a>
                                    </td>
                                </tr>
                        ...
                        <tbody>
                            @forelse($demandesConfirmees as $demande)
                                <tr>
                                    ...
                                    <td>{{ $demande->date_visite ? $demande->date_visite->format('d/m/Y H:i') : 'Non définie' }}</td>
                                    <td>
                                        <a href="{{ route('vendeur.demandes.show', $demande) }}" class="btn btn-info btn-sm" title="Voir">
                                            <i class="bi bi-eye"></i>
                                        </a>
                                        @if(!$demande->vente)
                                            <a href="{{ route('vendeur.ventes.create', $demande) }}" class="btn btn-success btn-sm" title="Créer vente">
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="text-center">Aucune demande confirmée</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
            ...
            <div class="tab-pane fade" id="annulees" role="tabpanel">
                <div class="table-responsive">
                    <table class="table table-striped">
                        ...
                        <tbody>
                            @forelse($demandesAnnulees as $demande)
                                <tr>
                                    <td>{{ $demande->created_at->format('d/m/Y H:i') }}</td>
                                    <td>{{ $demande->client->nom }} {{ $demande->client->prenom }}</td>
                                    <td>{{ $demande->voiture->marque }} {{ $demande->voiture->modele }}</td>
